...
Демо развёрнуто на Vercel (Next.js 16 + Neon Postgres).

**Админ:** `admin@cargohs54.ru` / `admin123` · **Клиент:** `test@test.ru` / `client123`

**Демо-профили клиентов** (пароль у всех `client123`): `test@test.ru` (Иван, Екатеринбург — 3 заявки с перепиской) · `annaivanova@mail.ru` (Анна, Москва — 3 заявки) · `dpetrov@yandex.ru` (Дмитрий, Санкт-Петербург) · `olga.guseva@gmail.com` (Ольга, Новосибирск)

В базе 10 заявок во всех статусах: Новая → Принята → Отправлена → Доставлена / Отменена, с товарами, чатом клиент-админ и заметками менеджера.
