...
**Демо-профили клиентов** (пароль у всех `client123`): `annaivanova@mail.ru` (Анна, Москва — 3 заявки с перепиской) · `dpetrov@yandex.ru` (Дмитрий, Санкт-Петербург) · `olga.guseva@gmail.com` (Ольга, Новосибирск)

В базе 7 заявок во всех статусах: Новая → Принята → Отправлена → Доставлена / Отменена, с товарами, чатом клиент-админ и заметками менеджера.
