...
**Demo client profiles** (password `client123`): `test@test.ru` (Ivan, Yekaterinburg — 3 orders with chat) · `annaivanova@mail.ru` (Anna, Moscow — 3 orders) · `dpetrov@yandex.ru` (Dmitry, St. Petersburg) · `olga.guseva@gmail.com` (Olga, Novosibirsk)

The database contains 10 orders covering every status: New → Approved → Shipped → Delivered / Cancelled, with order items, client-admin chat and manager notes.
